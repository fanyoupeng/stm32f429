...
UINT16 W25QXX_TYPE=W25Q32;	//默认是W25Q32
...
//    UINT8 temp;
    ...
    __HAL_RCC_GPIOF_CLK_ENABLE();           //使能GPIOE时钟
	...
	SPI4_Init();		   			        //初始化SPI
	SPI4_SetSpeed(SPI_BAUDRATEPRESCALER_2); //设置为45M时钟,高速模式
...
//		{
//			W25QXX_CS=0; 			        //选中
//			SPI4_ReadWriteByte(W25X_Enable4ByteAddr);//发送进入4字节地址模式指令   
//			W25QXX_CS=1;       		        //取消片选   
//		}
//    }
}  

UINT32 spi_flash_init()
{
	UINT16 flashId;
	W25QXX_Init();
	flashId = W25QXX_ReadID();
	if(flashId == W25Q32) {
		printf("spi flash(W25Q32) ID(0x%x) init success!\n", flashId);
		return TRUE;
	}
	else {
		printf("spi flash(W25Q32) ID(0x%x) init failed!\n", flashId);
		return FALSE;
	}
}
...
{ 
    ...
    {
        SPI4_ReadWriteByte((UINT8)((ReadAddr)>>24));    
    }
    SPI4_ReadWriteByte((UINT8)((ReadAddr)>>16));   //发送24bit地址    
    SPI4_ReadWriteByte((UINT8)((ReadAddr)>>8));   
    SPI4_ReadWriteByte((UINT8)ReadAddr);   
...
void W25QXX_Write_Page(UINT8* pBuffer,UINT32 WriteAddr,UINT16 NumByteToWrite)
{
    ...
    if(W25QXX_TYPE==W25Q256)                //如果是W25Q256的话地址为4字节的，要发送最高8位
    {
        SPI4_ReadWriteByte((UINT8)((WriteAddr)>>24)); 
    }
    SPI4_ReadWriteByte((UINT8)((WriteAddr)>>16)); //发送24bit地址    
    SPI4_ReadWriteByte((UINT8)((WriteAddr)>>8));   
    SPI4_ReadWriteByte((UINT8)WriteAddr);   
...
{  
    ...
    if(W25QXX_TYPE==W25Q256)                //如果是W25Q256的话地址为4字节的，要发送最高8位
    {
        SPI4_ReadWriteByte((UINT8)((Dst_Addr)>>24)); 
    }
    SPI4_ReadWriteByte((UINT8)((Dst_Addr)>>16));  //发送24bit地址    
    SPI4_ReadWriteByte((UINT8)((Dst_Addr)>>8));   
    SPI4_ReadWriteByte((UINT8)Dst_Addr);  
	W25QXX_CS=1;                            //取消片选     	      
